...
pthread_function_caller_t pthread_caller_func = nullptr;
pthread_create_impl_t pthread_create_impl = nullptr;
pthread_key_create_impl_t pthread_key_create_impl = nullptr;
pthread_key_delete_impl_t pthread_key_delete_impl = nullptr;
pthread_getspecific_impl_t pthread_getspecific_impl = nullptr;
pthread_setspecific_impl_t pthread_setspecific_impl = nullptr;

void shim::libc_shim_pthread_init(pthread_function_caller_t new_pthread_caller_func, pthread_create_impl_t new_pthread_create_impl, 
...
{
    pthread_caller_func = new_pthread_caller_func;
    pthread_create_impl = new_pthread_create_impl;
    pthread_key_create_impl = new_pthread_key_create_impl;
    pthread_key_delete_impl = new_pthread_key_delete_impl;
    pthread_getspecific_impl = new_pthread_getspecific_impl;
    pthread_setspecific_impl = new_pthread_setspecific_impl;
}

bionic::pthread_cleanup_holder::~pthread_cleanup_holder() {
    auto p = current;
    while (p) {
        if (pthread_caller_func) {
            pthread_caller_func((void*)p->routine, true, p->arg);
        } else {
            p->routine(p->arg);
        }
        p = p->prev;
...
int shim::pthread_create(pthread_t *thread, const shim::pthread_attr_t *attr, void *(*fn)(void *), void *arg) {
    ...
    assert(pthread_create_impl);
    int ret = pthread_create_impl(thread, &hattr.attr, fn, arg);
...
int shim::pthread_attr_setguardsize(shim::pthread_attr_t *attr, size_t value) {
    attr->guard_size = value;
    return 0;
}

int shim::pthread_attr_getguardsize(shim::pthread_attr_t *attr, size_t *value) {
...
int shim::pthread_key_create(pthread_key_t *key, void (*destructor)(void *)) {
    ...
    assert(pthread_key_create_impl);
    int ret = pthread_key_create_impl(&host_key, destructor);
    if (host_key > INT_MAX)
...
int shim::pthread_key_delete(pthread_key_t key) {
    assert(pthread_key_delete_impl);
    int ret = pthread_key_delete_impl((::pthread_key_t) key);
    return bionic::translate_errno_from_host(ret);
}

int shim::pthread_setspecific(pthread_key_t key, const void *value) {
    assert(pthread_setspecific_impl);
    int ret = pthread_setspecific_impl((::pthread_key_t) key, (void*)value);
    return bionic::translate_errno_from_host(ret);
}
...
void shim::pthread_cleanup_pop_impl(shim::bionic::pthread_cleanup_t *c, int execute) {
    ...
    if (execute) {
        if (pthread_caller_func) {
            pthread_caller_func((void*)c->routine, true, c->arg);
        } else {
            c->routine(c->arg);
        }
    }
...
int shim::pthread_once(pthread_once_t *control, void (*routine)()) {
    ...
    if (*control == 0) {
        *control = 1;
        if (pthread_caller_func) {
            pthread_caller_func((void*)routine, false, nullptr);
        } else {
            routine();
        }
    }
...
void shim::add_pthread_shimmed_symbols(std::vector<shimmed_symbol> &list) {
    list.insert(list.end(), {
        ...
        {"pthread_getspecific", pthread_getspecific_impl},

        {dynamic_symbol::Tag::HasExecutionSideEffects, "__pthread_cleanup_push", pthread_cleanup_push_impl},
        {dynamic_symbol::Tag::HasExecutionSideEffects, "__pthread_cleanup_pop", pthread_cleanup_pop_impl},

        {dynamic_symbol::Tag::HasExecutionSideEffects, "pthread_once", pthread_once},
