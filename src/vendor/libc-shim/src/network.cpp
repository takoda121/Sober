...
int bionic::to_host_sockopt_ip(int opt) {
    switch (opt) {
        ...
        default: {
            std::cout << "libc-shim WARN: unknown ip sockopt: " << opt << std::endl;
            return opt;
        };
    }
}
...
int bionic::to_host_sockopt(int level, int opt) {
    ...
    switch (level) {
        ...
        case SOL_TCP: return opt; // TODO?
        default: {
            std::cerr << "Unknown sockopt level: " << level << std::endl;
            throw std::runtime_error("Unknown sockopt level");
        }
    }
}
