...
clockid_t bionic::to_host_clock_type(bionic::clock_type type) {
    switch (type) {
        ...
        case clock_type::MONOTONIC_RAW: return CLOCK_MONOTONIC_RAW;
#ifdef __APPLE__
...
int bionic::to_host_mmap_flags(bionic::mmap_flags flags) {
    if (((uint32_t) flags & ~((uint32_t) mmap_flags::FIXED | (uint32_t) mmap_flags::ANON |
        ...
        std::cerr << "Unsupported mmap flags " << std::hex << static_cast<int>(flags) << std::endl;
...
ssize_t shim::__readlink_chk(const char* path, char* buf, size_t len, size_t buflen) {
    if (len > buflen) {
        fprintf(stderr, "detected readlink past buffer size");
        abort();
    }
...
long shim::fakesyscall(long sysno, ...) {
    if (sysno == FAKE_SYS_gettid) {
        ...
        va_end(l);
        return res;
    } else if (sysno == FAKE_SYS_futex) {
        va_list l;
        va_start(l, sysno);
        uint32_t *uaddr = va_arg(l, uint32_t*);
        int futex_op = va_arg(l, int);
        uint32_t val = va_arg(l, uint32_t);
        const struct timespec *timeout = va_arg(l, const struct timespec*);
        uint32_t *uaddr2 = va_arg(l, uint32_t*);
        uint32_t val3 = va_arg(l, uint32_t);
        int res = syscall(SYS_futex, uaddr, futex_op, val, timeout, uaddr2, val3);
        va_end(l);
        return res;
    }
    std::cout << "Unimplemented libc syscall " << sysno << std::endl;
    return ENOSYS;
}
...
atexit_impl_t atexit_impl = nullptr;
qsort_impl_t qsort_impl = nullptr;
bsearch_impl_t bsearch_impl = nullptr;
setjmp_impl_t setjmp_impl = nullptr;
longjmp_impl_t longjmp_impl = nullptr;
void shim::libc_shim_common_init(atexit_impl_t new_atexit_impl, qsort_impl_t new_qsort_impl, bsearch_impl_t new_bsearch_impl, setjmp_impl_t new_setjmp_impl, longjmp_impl_t new_longjmp_impl) {
    atexit_impl = new_atexit_impl;
    qsort_impl = new_qsort_impl;
    bsearch_impl = new_bsearch_impl;
    setjmp_impl = new_setjmp_impl;
    longjmp_impl = new_longjmp_impl;
}
...
void shim::add_stdlib_shimmed_symbols(std::vector<shim::shimmed_symbol> &list) {
    list.insert(list.end(), {
        {"abort", +[]() { assert(!"abort is not implemented"); }},
        {"atexit", +[]() { assert(!"atexit is not implemented (__cxa_atexit should have been called instead)"); }},
        {"exit", +[]() { assert(!"exit is not implemented"); }},
        {"_Exit", +[]() { assert(!"_Exit is not implemented"); }},
        ...
        {"realpath", realpath},
        {dynamic_symbol::Tag::HasExecutionSideEffects, "bsearch", bsearch_impl},
        {dynamic_symbol::Tag::HasExecutionSideEffects, "qsort", qsort_impl},
        {"mblen", mblen},
        {"mbtowc", mbtowc},
        {"wctomb", wctomb},
        {"mbstowcs", mbstowcs},
...
void shim::add_unistd_shimmed_symbols(std::vector<shim::shimmed_symbol> &list) {
    list.insert(list.end(), {
        ...
        {"execv", +[]() { assert(!"execv is not implemented"); }},
        {"execve", +[]() { assert(!"execve is not implemented"); }},
        {"execle", +[]() { assert(!"execle is not implemented"); }},
        {"execl", +[]() { assert(!"execl is not implemented"); }},
        {"execvp", +[]() { assert(!"execvp is not implemented"); }},
        {"execlp", +[]() { assert(!"execlp is not implemented"); }},
        ...
        {"fork", +[]() { assert(!"fork is not implemented"); }},
        {"vfork", +[]() { assert(!"vfork is not implemented"); }},
        ...
        {"pread64", WithErrnoUpdate(pread64)},
        {"pwrite", WithErrnoUpdate(pwrite)},
        {"pwrite64", WithErrnoUpdate(pwrite64)},
    });
}
...
void shim::add_signal_shimmed_symbols(std::vector<shim::shimmed_symbol> &list) {
    list.insert(list.end(), {
        ...
        {"pthread_sigmask", +[](int how, const sigset_t *set, sigset_t *oldset) -> int {
            return 0;
        }},
...
void shim::add_string_shimmed_symbols(std::vector<shim::shimmed_symbol> &list) {
    list.insert(list.end(), {
        ...
        {"tolower_l", ::tolower_l},
        {"toupper_l", ::toupper_l},
        {"memchr", (void* (*)(void *s, int c, size_t n))::memchr},
        {"memrchr", (void* (*)(void *s, int c, size_t n))::memrchr},
...
void shim::add_setjmp_shimmed_symbols(std::vector<shim::shimmed_symbol> &list) {
    list.insert(list.end(), {
        {dynamic_symbol::Tag::HasExecutionSideEffects, "setjmp", setjmp_impl},
        {dynamic_symbol::Tag::HasExecutionSideEffects, "longjmp", longjmp_impl},
        {dynamic_symbol::Tag::HasExecutionSideEffects, "sigsetjmp", +[](uint64_t *env, int) -> int {
            // TODO: this is a stub.
            return setjmp_impl(env);
        }},
        {dynamic_symbol::Tag::HasExecutionSideEffects, "siglongjmp", +[](u64 *env, int value) {
            // TODO: this is a stub.
            longjmp_impl(env, value);
        }},
    });
}
