...
void A64EmitX64::EmitA64CallSupervisor(A64EmitContext& ctx, IR::Inst* inst) {
    ...
    if (fast_call) {
        ...
        assert(fast_call->float_params <= ABI_FLOAT_PARAM_COUNT);

        for (std::uint32_t i = 0; i < fast_call->integer_params; i++) {
            // ARM register x[i]
            const auto addr = qword[r15 + offsetof(A64JitState, reg) + sizeof(u64) * static_cast<size_t>(i)];
            code.mov(code.ABI_PARAMS[i], addr);
        }

        for (std::uint32_t i = 0; i < fast_call->float_params; i++) {
            // ARM register d[i]
            const auto addr = qword[r15 + offsetof(A64JitState, vec) + sizeof(u64) * 2 * static_cast<size_t>(i)];
            code.movq(code.ABI_FLOAT_PARAMS[i], addr);
        }

        code.CallFunction(fast_call->function);

        using ReturnType = Dynarmic::A64::UserCallbacks::SupervisorFastCall::ReturnType;
        switch (fast_call->return_type) {
            case ReturnType::Integer: {
                // ARM register x0 is the integer return
                const auto addr = qword[r15 + offsetof(A64JitState, reg) + sizeof(u64) * static_cast<size_t>(0)];
                code.mov(addr, code.ABI_RETURN);
            } break;

            case ReturnType::Float: {
                // ARM register d0 is the floating point return
                const auto addr = qword[r15 + offsetof(A64JitState, vec) + sizeof(u64) * 2 * static_cast<size_t>(0)];
                code.movq(addr, code.ABI_FLOAT_RETURN);
            } break;

            case ReturnType::Void: break;
        }
    } else {
